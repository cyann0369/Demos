...
graphiques_uni <- function(data, variable) {
  ...
  boxplot <- ggplot(data, aes_string(x = "1", y = variable)) +
    ...
    stat_summary(fun = mean, geom = "point", shape = 16, size = 2, color = "green", fill = "green") +
...
pca_result <- PCA(df_stand, scale.unit = TRUE, graph = TRUE)
pca_result$var
...
modele_lm <- lm(txabs ~ Salairemoy + TxPauv + NonDiplome + txcho + V1 + V2 + V3, df_stand)
...
modele_pcr <- pcr(txabs ~ Salairemoy  + TxPauv + NonDiplome + txcho + V1 + V2 + V3, 
                  df = df_stand, scale = TRUE, ncomp = 2)
