...
plot(eig_values[,3],type = "b", col = "blue",
     xlab = "Nombre de Composantes", ylab = "Variance Cumulée (%)",
     main = "Pourcentage Cumulé de Variance Expliquée")
axis(1, at = seq(1, nrow(eig_values), by = 1))

bar_pos <- barplot(eig_values[,3],
        names.arg = rep("", nrow(eig_values)),  # Supprime les noms par défaut
        col = "blue",
        xlab = "Nombre de Composantes",
        ylab = "Variance Cumulée (%)",
        main = "Pourcentage Cumulé de Variance Expliquée")
axis(1, at = bar_pos, labels = 1:nrow(eig_values))
text(x = bar_pos, 
     y = eig_values[,3],   # Position des textes : au-dessus des barres
     labels = round(eig_values[,3], 1),  # Texte arrondi à 1 décimale
     pos = 1,   # Position au-dessus de la barre (3 = au-dessus)
     cex = 0.8, # Taille du texte
     col = "white")  # Couleur du texte
```
```{r}
df_pca <- pca_result$ind$coord
```
...
draw_plot_tsne_virgin <- function(){
  set.seed(123)
...
draw_plot_tsne <- function(cluster_points){
  set.seed(123)
...
km_silhouette_score_mean <- function(k){
  km <- kmeans(df_pca, centers = k, nstart=25)
...
```{r}
km <- kmeans(df_pca, centers = 3, nstart=25)
draw_plot_tsne(km$cluster)

```

```{r}
km <- kmeans(df_pca, centers = 5, nstart=25)
draw_plot_tsne(km$cluster)
...
draw_boxplot_per_class <- function(df,cluster_points,varname){
  ...
  ggplot(agg_df, aes_string(x="cluster", y=varname,group="cluster")) +
    geom_boxplot(fill = "lightblue", outlier.colour = "red")+
    geom_point(aes_string(x = "cluster", y = varname),col = "red", alpha = 0.2)+
    scale_x_discrete("Cluster",limits = factor(c(x_labels)))+
    ggtitle(paste("Boxplot par cluster pour la variable",varname))
}
for (column in colnames(df[,-c(1,2)])){
    plot(draw_boxplot_per_class(df,km$cluster,column))
}
```
...
```{r}
df_pca.dist <- stats::dist(df_pca)
cah.ward <- hclust(df_pca.dist,method="ward.D2")
```

```{r}
cah_silhouette_score_mean <- function(k){
  clusters <- cutree(cah.ward, k = k)
  ss <- silhouette(clusters, df_stand.dist)
  mean(ss[, 3])
}

k <- 2:10
avg_sil <- base::sapply(k,cah_silhouette_score_mean)
plot(k, type='b', avg_sil,
     xlab='Nombre de clusters', 
     ylab='Score Silhouette Moyen', 
     frame=FALSE)
...
```{r}
fviz_nbclust(df_pca, FUN=hcut, method='silhouette')
```
...
draw_plot_tsne(clusters)
```

```{r}
for (column in colnames(df[,-c(1,2)])){
    plot(draw_boxplot_per_class(df,clusters,column))
}
```
...
nearest_neighbors <- get.knn(df_pca, k = 2)
...
```{r}
dbs <- dbscan(df_pca,eps=2)
sil <- silhouette(dbs$cluster, df_stand.dist)
plot(fviz_silhouette(sil))
draw_plot_tsne(dbs$cluster)
```
